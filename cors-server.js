...
app.post('/order/new', async (req, res) => {
  ...
  try {
    ...
    const insertItemText = `
      INSERT INTO order_items (itemid, quantity)
      VALUES ($1, $2);
    `;

    for (const item of cart) {
      await client.query(insertItemText, [item.itemid, item.quantity]);
    }

    res.status(201).json({ data: { message: 'Order items have been added successfully' } });
  } catch (err) {
